    ...
    <aside className="w-full h-full bg-gray-900 border-r border-gray-800 flex flex-col">
      ...
      <div className="p-3 border-b border-gray-800">
        {showNewNote ? (
          ...
          <button onClick={() => setShowNewNote(true)} className="w-full text-sm bg-gray-800 text-gray-300 py-2 rounded-lg hover:bg-gray-700 border border-gray-700 border-dashed">
            + New Note
          </button>
        )}
      </div>
              ...
              {note.frontmatter?.tags?.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-1.5">
                  {note.frontmatter.tags.slice(0, 3).map(tag => (
                    <span key={tag} className="text-[10px] px-1.5 py-0.5 rounded-full" style={{ backgroundColor: getTagColor(tag) + '20', color: getTagColor(tag) }}>
                      {tag}
                    </span>
                  ))}
                  {note.frontmatter.tags.length > 3 && (
                    <span className="text-[10px] text-gray-500">+{note.frontmatter.tags.length - 3}</span>
                  )}
                </div>
              )}
